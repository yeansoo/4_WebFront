...
document.getElementById("btn1").addEventListener("click",function(){
    ...
    // 3. for(item of 배열(또는 컬랙션)){ } - 배열, 컬랙션
    ...
    // 4. for(let key in 객체) - JS 객체용 for 구문
});
...
document.getElementById("btn2").addEventListener("click",function(){
    ...
    console.log("arr : "+arr.toString()); // toString() 생략 가능

    // 배열.pop() : 배열의 마지막 요소를 꺼내와 반환
    const temp=arr.pop(); // 마지막 요소("삼겹살")를 꺼내와 temp 변수에 확인
...
document.getElementById("btn3").addEventListener("click",function(){
    ...
    console.log("로또 번호 : "+lotto.toString());
})