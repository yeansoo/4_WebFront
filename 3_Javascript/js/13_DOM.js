...
document.getElementById("btn1").addEventListener("click",function(){
    ...
    // #test의 자식 노드 중 5번 인덱스 노드의 이전 형제 노드
    console.log(test.childNodes[5].previousSibling);

    // #test의 자식 노드 중 5번 인덱스 노드의 다음 형제 노드
    console.log(test.childNodes[5].nextSibling);

    // 노드 탐색 구문 연속 작성 

    // #test의 자식 노드 중 5번 인덱스 노드의 이전 형제 노드의 이전 형제노드 
...
document.getElementById("btn2").addEventListener("click",function(){
    ...
    // #test의 자식요소 중 2번 인덱스 이전 형제 요소 클릭 시
    // 작성된 내용을 alert로 출력
...
btn4.addEventListener("click",function(){
   if(count4<=10){
        ...
        // div4의 내용으로 count4 추가 후 1증가
        div.innerText+=count4;
        count4++;

        // input을 div의 마지막 자식으로 추가 
        // 요소.append(노드 또는 마지막요소)
        // -> 요소A의 마지막 자식으로 추가 
        div.append(input);
